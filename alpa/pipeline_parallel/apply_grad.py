...
def _insert_to_pipeline_marker(marker, new_inv, mapping):
    ...
    return new_jaxpr_eqn(invs, outvs, marker.primitive, marker.params,
                         marker.source_info)
    ...
    for eqn in eqns:
        if eqn.primitive is not pipeline_p:
            ...
            for eq in appended_eqns:
                ...
                # create the new eqn
                new_eqns.append(
                    new_jaxpr_eqn(eq_new_invs, eq_new_outvs, eq.primitive,
                                  eq.params, eq.source_info))
    ...
    for eqn in reversed(compute_eqns):
        ...
        if eqn.primitive is pipeline_p:
            ...
            if eqn.params['mark_type'] == 'end':
                ...
                assert marked_vars.issuperset(outvars)
            new_compute_eqns.append(eqn)
        elif marked_vars.issuperset(outvars):
            # eqn is marked if all outvars are marked, its invars are marked.
            marked_vars.update(invars)
            new_compute_eqns.append(eqn)
        else:
